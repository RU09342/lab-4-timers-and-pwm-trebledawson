...
void main(void)
{
    ...
    P5IE |= BUTTON;             // Set P1.3 as interrupt pin
...
__interrupt void button_press(void) // Define interrupt function that runs whenever interrupt is detected
{
    ...
    P5IE &= !BUTTON; // Turn off P1.3 interrupt
...
__interrupt void button_debounce_timer(void)
{
    ...
    P5IE |= BUTTON;             // Enable interrupt code
    P5IFG = ~BUTTON;            // Disable interrupt flag to re-enable interrupt
}
