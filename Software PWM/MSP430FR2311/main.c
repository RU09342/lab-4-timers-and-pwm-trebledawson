...
#pragma vector=TIMER0_B0_VECTOR // When TB0R reaches CCR0
__interrupt void led_period(void)
{
    P1OUT |= LED1;   // Turn P1.6 LED on
}

#pragma vector=TIMER0_B1_VECTOR // When TB0R reaches CCR1
...
__interrupt void button_press(void) // Define interrupt function that runs whenever interrupt is detected
{
    ...
    TB1CTL = TBSSEL_1 + MC_1 + ID_3 + TBCLR; // Set Timer B0 to BCLK, up mode
    TB1CCTL0 = CCIE;  // Compare interrupt enable
    TB1CCR0 = 1000;  // Delay
}
...
{
    TB1CTL = MC_0;            // Halt timer
    TB1R = 0;                 // Reset count register
...
void TimerB0Setup()
{
    ...
    TB0CCTL1 = CCIE ;
    TB0CCR1 = TB0CCR0 / 2;           // Initialize TB0CCR1 to 50% of TB0CCR0
                                     // i.e., 50% PWM
}
